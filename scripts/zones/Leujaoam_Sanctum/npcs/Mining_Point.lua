...
entity.onTrigger = function(player, npc)
    ...
    if npc:checkDistance(player) > 3 then
        ...
        return
    elseif not player:hasItem(605, tpz.inventoryLocation.TEMPITEMS) then -- Pickaxe
        player:messageSpecial(ID.text.MINE_NO_PICK, 605)
        return
    elseif instance:getEntity(bit.band(mineralEater, 0xFFF), tpz.objType.MOB):isSpawned() or npc:getLocalVar("Wait") >= currentTime then
        player:messageSpecial(ID.text.CANT_MINE)
        return
    end

    if chance >= 30 then
        player:sendEmote(npc, tpz.emote.EXCAVATION, tpz.emoteMode.MOTION, true)
        ...
        if chance > 900 then -- items
            if chance > 990 then
                ...
                player:timer(3000, function(player)
                player:addTreasure(17296, instance:getEntity(bit.band(17060016, 0xFFF), tpz.objType.NPC)) end) -- Pebble
            end
        ...
        if pickBreak >= 900 then
            ...
            player:delItem(605, 1, tpz.inventoryLocation.TEMPITEMS) end)
        end
    else
