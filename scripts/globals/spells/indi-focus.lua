...
function onMagicCastingCheck(caster, target, spell)
    if caster:hasStatusEffect(tpz.effect.COLURE_ACTIVE) then
        ...
        if effect:getSubType() == tpz.effect.ELEMENTAL_SEAL then
...
function onSpellCast(caster, target, spell)
    ...
    caster:addStatusEffectEx(tpz.effect.COLURE_ACTIVE, tpz.effect.COLURE_ACTIVE, 6, 3, 180, tpz.effect.ELEMENTAL_SEAL, power, tpz.auraTarget.ALLIES, tpz.effectFlag.AURA)
