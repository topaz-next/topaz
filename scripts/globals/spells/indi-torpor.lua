...
function onMagicCastingCheck(caster, target, spell)
    if caster:hasStatusEffect(tpz.effect.COLURE_ACTIVE) then
        ...
        if effect:getSubType() == tpz.effect.EVASION_DOWN then
...
function onSpellCast(caster, target, spell)
    ...
    caster:addStatusEffectEx(tpz.effect.COLURE_ACTIVE, tpz.effect.COLURE_ACTIVE, 13, 3, 180, tpz.effect.EVASION_DOWN, power, tpz.auraTarget.ENEMIES, tpz.effectFlag.AURA)
