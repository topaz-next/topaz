...
bool CLatentEffectContainer::ProcessLatentEffect(CLatentEffect& latentEffect)
{
    ...
    switch (latentEffect.GetConditionsID())
    {
        ...
        case LATENT_SUBJOB:
            ...
            break;
        case LATENT_PET_ID:
            expression = m_POwner->PPet != nullptr && m_POwner->PPet->objtype == TYPE_PET &&
                         (dynamic_cast<CPetEntity*>(m_POwner->PPet))->m_PetID == latentEffect.GetConditionsValue();
            break;
        case LATENT_WEAPON_DRAWN:
        ...
        case LATENT_AVATAR_IN_PARTY:
            if (m_POwner->PParty != nullptr)
            {
                for (auto* member : m_POwner->PParty->members)
                {
                    ...
                    {
                        auto* PPet = dynamic_cast<CPetEntity*>(member->PPet);
            ...
            {
                auto* PPet = dynamic_cast<CPetEntity*>(m_POwner->PPet);
        ...
        case LATENT_JOB_IN_PARTY:
            if (m_POwner->PParty != nullptr)
            {
                for (auto* member : m_POwner->PParty->members)
                {
                    ...
                    for (auto* trust : dynamic_cast<CCharEntity*>(member)->PTrusts)
                    {
        ...
        case LATENT_WEAPON_BROKEN:
        {
            ...
            auto* item = dynamic_cast<CItemWeapon*>(m_POwner->getEquip((SLOTTYPE)slot));
            switch (slot)
            {
                case SLOT_MAIN:
                case SLOT_SUB:
