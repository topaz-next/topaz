...
void CParty::DisbandParty(bool playerInitiated)
{
    ...
    if (m_PartyType == PARTY_PCS)
    {
        ...
        {
            CCharEntity* PChar = dynamic_cast<CCharEntity*>(member);
            PChar->ClearTrusts();
...
uint8 CParty::MemberCount(uint16 ZoneID)
{
    ...
    for (auto* member : members)
    {
        ...
        {
            auto* charMember = dynamic_cast<CCharEntity*>(member);
...
void CParty::PushMember(CBattleEntity* PEntity)
{
    ...
    ReloadTreasurePool(dynamic_cast<CCharEntity*>(PEntity));
}
        ...
        for (auto& member : members)
        {
            ...
            for (auto&& memberinfo : info)
            {
                ...
                if (PPartyMember)
                {
                    ...
                    if (PLeader != nullptr)
                    {
                        for (auto* PTrust : (dynamic_cast<CCharEntity*>(PLeader))->PTrusts)
                        {
                            j++;
    ...
    if (PChar->PParty != nullptr)
    {
        if (PChar->PParty->m_PAlliance != nullptr)
        {
            for (uint8 a = 0; a < PChar->PParty->m_PAlliance->partyList.size(); ++a)
            {
                ...
                {
                    CCharEntity* PPartyMember = dynamic_cast<CCharEntity*>(PChar->PParty->m_PAlliance->partyList.at(a)->members.at(i));
        ...
        else if (PChar->PParty->m_PAlliance == nullptr)
        {
            for (auto& member : members)
            {
                CCharEntity* PPartyMember = dynamic_cast<CCharEntity*>(member);
...
void CParty::SetSyncTarget(int8* MemberName, uint16 message)
{
    ...
    if (map_config.level_sync_enable)
    {
        if (PEntity && PEntity->objtype == TYPE_PC)
        {
            ...
            {
                (dynamic_cast<CCharEntity*>(GetLeader()))
                    ->pushPacket(new CMessageBasicPacket(dynamic_cast<CCharEntity*>(GetLeader()), dynamic_cast<CCharEntity*>(GetLeader()), 0, 10, 541));
                return;
            }
            if (PChar->getZone() != GetLeader()->getZone())
            {
                (dynamic_cast<CCharEntity*>(GetLeader()))
                    ->pushPacket(new CMessageBasicPacket(dynamic_cast<CCharEntity*>(GetLeader()), dynamic_cast<CCharEntity*>(GetLeader()), 0, 0, 542));
                return;
            }
            ...
            for (auto& member : members)
            {
                ...
                {
                    (dynamic_cast<CCharEntity*>(GetLeader()))
                        ->pushPacket(new CMessageBasicPacket(dynamic_cast<CCharEntity*>(GetLeader()), dynamic_cast<CCharEntity*>(GetLeader()), 0, 0, 543));
                    return;
                }
            }
            m_PSyncTarget = PChar;
            for (auto& i : members)
            {
                ...
                CCharEntity* member = dynamic_cast<CCharEntity*>(i);
...
{
    CCharEntity* sync      = dynamic_cast<CCharEntity*>(m_PSyncTarget);
