...
void CAttackRound::CreateAttacks(CItemWeapon* PWeapon, PHYSICAL_ATTACK_DIRECTION direction)
{
    ...
    {
        auto modSwings = std::min<uint8>((uint8) dynamic_cast<CCharEntity*>(m_attacker)->getMod(Mod::MAX_SWINGS), 8);
    ...
    {
        auto multiHitMax = (uint8) dynamic_cast<CMobEntity*>(m_attacker)->getMobMod(MOBMOD_MULTI_HIT);
...
void CAttackRound::CreateKickAttacks()
{
    if (m_attacker->objtype == TYPE_PC)
    {
        ...
        {
            kickAttack += (dynamic_cast<CCharEntity*>(m_attacker))->PMeritPoints->GetMeritValue(MERIT_KICK_ATTACK_RATE, dynamic_cast<CCharEntity*>(m_attacker));
        }
