...
void SmallPacket0x016(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    else
    {
        ...
        {
            PChar->pushPacket(new CCharPacket(dynamic_cast<CCharEntity*>(PEntity), ENTITY_SPAWN, UPDATE_ALL_CHAR));
        }
        else
        {
            if (!PEntity)
...
void SmallPacket0x032(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    CCharEntity* PTarget = dynamic_cast<CCharEntity*>(PChar->GetEntity(targid, TYPE_PC));
...
{
    TracyZoneScoped;
    CCharEntity* PTarget = dynamic_cast<CCharEntity*>(PChar->GetEntity(PChar->TradePending.targid, TYPE_PC));
    ...
    if (PTarget != nullptr && PTarget->id == PChar->TradePending.id)
    {
        ...
        if (PItem != nullptr && PItem->getID() == itemID && quantity + PItem->getReserve() <= PItem->getQuantity())
        {
            ...
            if (quantity > 0)
            {
                ...
                {
                    CItemLinkshell* PItemLinkshell  = dynamic_cast<CItemLinkshell*>(PItem);
        ...
        case 0x04:
        {
            ...
            if ((PItem != nullptr) && (PItem->getID() == itemid) && !(PItem->isSubType(ITEM_LOCKED)) && !(PItem->getFlag() & ITEM_FLAG_NOAUCTION))
            {
                if (PItem->isSubType(ITEM_CHARGED) &&
                    (dynamic_cast<CItemUsable*>(PItem))->getCurrentCharges() < (dynamic_cast<CItemUsable*>(PItem))->getMaxCharges())
                {
...
void SmallPacket0x050(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
        ...
        containerID != LOC_WARDROBE4)
    {
        ...
        if (containerID != LOC_MOGSATCHEL && containerID != LOC_MOGSACK && containerID != LOC_MOGCASE)
        {
            return;
        }
    }
...
void SmallPacket0x053(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    else if (type == 3)
    {
        ...
        for (int i = 0x08; i < 0x08 + (0x08 * count); i += 0x08)
        {
            ...
            else if (!((dynamic_cast<CItemEquipment*>(PItem))->getEquipSlotId() & (1 << equipSlotId)))
            {
                itemId = 0;
            }
            ...
            switch (equipSlotId)
            {
                ...
                case SLOT_RANGED:
                case SLOT_AMMO:
                    charutils::UpdateWeaponStyle(PChar, equipSlotId, dynamic_cast<CItemWeapon*>(PChar->getEquip((SLOTTYPE)equipSlotId)));
                case SLOT_HEAD:
                case SLOT_BODY:
                case SLOT_HANDS:
                case SLOT_LEGS:
        ...
        case 0: // party - must by party leader or solo
            if (PChar->PParty == nullptr || PChar->PParty->GetLeader() == PChar)
            {
                ...
                if (targid != 0)
                {
                    ...
                    {
                        PInvitee = dynamic_cast<CCharEntity*>(PEntity);
                    }
                }
                else
                {
...
void SmallPacket0x071(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    switch (data.ref<uint8>(0x0A))
    {
        case 0: // party - party leader may remove member of his own party
            if (PChar->PParty && PChar->PParty->GetLeader() == PChar)
            {
                CCharEntity* PVictim = dynamic_cast<CCharEntity*>(PChar->PParty->GetMemberByName(data[0x0C]));
                if (PVictim)
                {
        ...
        case 5: // alliance - alliance leader may kick a party by using that party's leader as kick parameter
            if (PChar->PParty && PChar->PParty->GetLeader() == PChar && PChar->PParty->m_PAlliance)
            {
                ...
                {
                    PVictim = dynamic_cast<CCharEntity*>(PChar->PParty->m_PAlliance->partyList[i]->GetMemberByName(data[0x0C]));
...
void SmallPacket0x0AA(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    CItemShop* item       = dynamic_cast<CItemShop*>(PChar->PGuildShop->GetItem(shopSlotID));
    CItem*     gil        = PChar->getStorage(LOC_INVENTORY)->GetItem(0);
...
void SmallPacket0x0AC(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    if (PChar->animation != ANIMATION_SYNTH)
    {
        if (PChar->PGuildShop != nullptr)
        {
            ...
            uint8      quantity   = data.ref<uint8>(0x07);
            uint8      shopSlotID = PChar->PGuildShop->SearchItem(itemID);
            CItemShop* shopItem   = dynamic_cast<CItemShop*>(PChar->PGuildShop->GetItem(shopSlotID));
...
void SmallPacket0x0C3(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    {
        CItemLinkshell* PItemLinkPearl = dynamic_cast<CItemLinkshell*>(itemutils::GetItem(515));
        if (PItemLinkPearl)
        {
...
void SmallPacket0x0C4(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    CItemLinkshell* PItemLinkshell = dynamic_cast<CItemLinkshell*>(PChar->getStorage(LocationID)->GetItem(SlotID));
    ...
    if (PItemLinkshell != nullptr && PItemLinkshell->isType(ITEM_LINKSHELL))
    {
        ...
        if (PItemLinkshell->getID() == 512)
        {
            ...
            if (LinkshellID != 0)
            {
                ...
                PItemLinkshell = dynamic_cast<CItemLinkshell*>(itemutils::GetItem(513));
...
void SmallPacket0x0D2(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    PChar->ForAlliance([PChar](CBattleEntity* PPartyMember) {
        if (PPartyMember->getZone() == PChar->getZone() && (dynamic_cast<CCharEntity*>(PPartyMember))->m_moghouseID == PChar->m_moghouseID)
        {
            PChar->pushPacket(new CPartyMapPacket(dynamic_cast<CCharEntity*>(PPartyMember)));
        }
    });
}
...
void SmallPacket0x0DD(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    if (type == 0x02)
    {
        if (PChar->id == id)
        {
            ...
            {
                int skill      = (dynamic_cast<CItemWeapon*>(PChar->getEquip(SLOT_RANGED)))->getSkillType();
                int bonusSkill = (dynamic_cast<CItemWeapon*>(PChar->getEquip(SLOT_RANGED)))->getILvlSkill();
                PChar->pushPacket(
            ...
            {
                int skill      = (dynamic_cast<CItemWeapon*>(PChar->getEquip(SLOT_AMMO)))->getSkillType();
                int bonusSkill = (dynamic_cast<CItemWeapon*>(PChar->getEquip(SLOT_AMMO)))->getILvlSkill();
                PChar->pushPacket(
                    new CMessageBasicPacket(PChar, PChar, PChar->RACC(skill, bonusSkill), PChar->RATT(skill, bonusSkill), MSGBASIC_CHECKPARAM_RANGE));
            }
            else
    ...
    else
    {
        ...
        switch (PEntity->objtype)
        {
            case TYPE_MOB:
            {
                CMobEntity* PTarget = dynamic_cast<CMobEntity*>(PEntity);
            ...
            case TYPE_PC:
            {
                CCharEntity* PTarget = dynamic_cast<CCharEntity*>(PEntity);
...
void SmallPacket0x0E8(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    switch (PChar->animation)
    {
        case ANIMATION_NONE:
        {
            ...
            if (PChar->PPet == nullptr ||
                (PChar->PPet->m_EcoSystem != SYSTEM_AVATAR && PChar->PPet->m_EcoSystem != SYSTEM_ELEMENTAL && !PChar->PAI->IsEngaged()))
            {
...
void SmallPacket0x0FA(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    CItemFurnishing* PItem = dynamic_cast<CItemFurnishing*>(PChar->getStorage(containerID)->GetItem(slotID));
...
void SmallPacket0x0FB(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    CItemFurnishing* PItem = dynamic_cast<CItemFurnishing*>(PItemContainer->GetItem(slotID));
...
void SmallPacket0x0FC(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    CItemFlowerpot* PPotItem          = dynamic_cast<CItemFlowerpot*>(PPotItemContainer->GetItem(potSlotID));
    if (PPotItem == nullptr)
    {
        return;
    }
...
void SmallPacket0x0FD(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    CItemContainer* PItemContainer = PChar->getStorage(containerID);
    CItemFlowerpot* PItem          = dynamic_cast<CItemFlowerpot*>(PItemContainer->GetItem(slotID));
    if (PItem == nullptr)
    {
        return;
    }
...
void SmallPacket0x102(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    if ((PChar->GetMJob() == JOB_BLU || PChar->GetSJob() == JOB_BLU) && job == JOB_BLU)
    {
        ...
        if (spellToAdd == 0x00)
        {
            for (uint8 i = 0x0C; i <= 0x1F; i++)
            {
                if (data.ref<uint8>(i) > 0)
                {
                    ...
                    spellIndex        = i - 0x0C;
                    CBlueSpell* spell = dynamic_cast<CBlueSpell*>(spell::GetSpell(
        ...
        else
        {
            ...
            {
                CBlueSpell* spell = dynamic_cast<CBlueSpell*>(
                    spell::GetSpell(static_cast<SpellID>(spellInQuestion + 0x200))); // the spells in this packet are offsetted by 0x200 from their spell IDs.
...
{
    TracyZoneScoped;
    CCharEntity* PTarget = dynamic_cast<CCharEntity*>(PChar->GetEntity(PChar->BazaarID.targid, TYPE_PC));
...
void SmallPacket0x105(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    CCharEntity* PTarget =
        charid != 0 ? (CCharEntity*)PChar->loc.zone->GetCharByID(charid) : dynamic_cast<CCharEntity*>(PChar->GetEntity(PChar->m_TargID, TYPE_PC));
...
void SmallPacket0x106(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    if ((PBazaarItem->getCharPrice() != 0) && (PBazaarItem->getQuantity() >= Quantity))
    {
        ...
        for (uint16 i = 0; i < PTarget->BazaarCustomers.size(); ++i)
        {
            CCharEntity* PCustomer = dynamic_cast<CCharEntity*>(PTarget->GetEntity(PTarget->BazaarCustomers[i].targid, TYPE_PC));
...
void SmallPacket0x10B(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    {
        CCharEntity* PCustomer = dynamic_cast<CCharEntity*>(PChar->GetEntity(PChar->BazaarCustomers[i].targid, TYPE_PC));
