...
int32 OnSpellCast(CBattleEntity* PCaster, CBattleEntity* PTarget, CSpell* PSpell)
{
    ...
    lua_prepscript(PSpell->getSpellGroup() == SPELLGROUP_BLUE
                       ? "scripts/globals/spells/bluemagic/%s.lua"
                       : PSpell->getSpellGroup() == SPELLGROUP_TRUST ? "scripts/globals/spells/trust/%s.lua" : "scripts/globals/spells/%s.lua",
                   PSpell->getName());
...
int32 ApplyZoneMixins(CBaseEntity* PMob)
{
    ...
    if (PMob->objtype == TYPE_MOB)
    {
        ...
        {
            CPetEntity* PPet = dynamic_cast<CPetEntity*>(PMob);
...
int32 OnMobEngaged(CBaseEntity* PMob, CBaseEntity* PTarget)
{
    ...
    int8 File[255];
    PMob->objtype == TYPE_PET ? snprintf((char*)File, sizeof(File), "scripts/globals/pets/%s.lua", dynamic_cast<CPetEntity*>(PMob)->GetScriptName().c_str())
                              : snprintf((char*)File, sizeof(File), "scripts/zones/%s/mobs/%s.lua", PMob->loc.zone->GetName(), PMob->GetName());

    if (PTarget->objtype == TYPE_PC)
    {
        (dynamic_cast<CCharEntity*>(PTarget))->m_event.reset();
        (dynamic_cast<CCharEntity*>(PTarget))->m_event.Target = PMob;
        (dynamic_cast<CCharEntity*>(PTarget))->m_event.Script.insert(0, (const char*)File);
    }
    ...
    else
    {
        ...
        switch (PMob->objtype)
        {
            case TYPE_MOB:
                ...
                break;
            case TYPE_PET:
                snprintf((char*)File, sizeof(File), "scripts/globals/pets/%s.lua", dynamic_cast<CPetEntity*>(PMob)->GetScriptName().c_str());
                break;
            case TYPE_TRUST:
                                                 ...
                                                 CBattleEntity* taChar)
{
    ...
    if (criticalHit)
    {
        luautils::OnCriticalHit(dynamic_cast<CBattleEntity*>(PMob), (CBattleEntity*)PChar);
    }
...
int32 OnPetAbility(CBaseEntity* PTarget, CBaseEntity* PMob, CMobSkill* PMobSkill, CBaseEntity* PMobMaster, action_t* action)
{
    ...
    if (PMob->objtype == TYPE_PET && map_config.skillup_bloodpact)
    {
        ...
        {
            CCharEntity* PMaster = dynamic_cast<CCharEntity*>(PPet->PMaster);
