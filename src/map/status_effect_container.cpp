...
bool CStatusEffectContainer::AddStatusEffect(CStatusEffect* PStatusEffect, bool silent)
{
    ...
    if (CanGainStatusEffect(PStatusEffect))
    {
        ...
        {
            puppetutils::CheckAttachmentsForManeuver(dynamic_cast<CCharEntity*>(m_POwner), PStatusEffect->GetStatusID(), true);
        }
        ...
        {
            CCharEntity* PChar = dynamic_cast<CCharEntity*>(m_POwner);
...
void CStatusEffectContainer::RemoveStatusEffect(CStatusEffect* PStatusEffect, bool silent)
{
    if (!PStatusEffect->deleted)
    {
        if (PStatusEffect->GetStatusID() >= EFFECT_FIRE_MANEUVER && PStatusEffect->GetStatusID() <= EFFECT_DARK_MANEUVER && m_POwner->objtype == TYPE_PC)
        {
            puppetutils::CheckAttachmentsForManeuver(dynamic_cast<CCharEntity*>(m_POwner), PStatusEffect->GetStatusID(), false);
        }
...
void CStatusEffectContainer::TickRegen(time_point tick)
{
    ...
    if (!m_POwner->isDead())
    {
        ...
        if (m_POwner->objtype == TYPE_PC)
        {
            PChar = dynamic_cast<CCharEntity*>(m_POwner);
        }
        ...
        if (m_POwner->PPet && (dynamic_cast<CPetEntity*>(m_POwner->PPet))->getPetType() == PETTYPE_AUTOMATON)
        {
            (dynamic_cast<CAutomatonEntity*>(m_POwner->PPet))->burdenTick();
        }
    }
}
