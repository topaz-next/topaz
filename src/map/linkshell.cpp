    ...
    if (newId == 514 || newId == 515)
    {
        for (auto& member : members)
        {
            if (strcmp((const char*)MemberName, (const char*)member->GetName()) == 0)
            {
                ...
                {
                    CItemLinkshell* newShellItem = dynamic_cast<CItemLinkshell*>(itemutils::GetItem(newId));
    ...
    for (auto& member : members)
    {
        if (strcmp((const char*)MemberName, (const char*)member->GetName()) == 0)
        {
            ...
            for (uint8 LocationID = 0; LocationID < MAX_CONTAINER_ID; ++LocationID)
            {
                ...
                {
                    CItemLinkshell* newPItemLinkshell = dynamic_cast<CItemLinkshell*>(Inventory->GetItem(SlotID));
...
uint32 RegisterNewLinkshell(const int8* name, uint16 color)
{
    ...
    {
        if (Sql_Query(SqlHandle, "INSERT INTO linkshells (name, color, postrights) VALUES ('%s', %u, %u)", name, color, static_cast<uint8>(LSTYPE_PEARLSACK)) !=
            SQL_ERROR)
        {
