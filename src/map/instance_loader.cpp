...
CInstanceLoader::CInstanceLoader(uint8 instanceid, CZone* PZone, CCharEntity* PRequester)
{
    ...
    zone                = PZone;
    CInstance* instance = (dynamic_cast<CZoneInstance*>(PZone))->CreateInstance(instanceid);
        ...
        if (task.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready)
        {
            ...
            else
            {
                ...
                for (auto PMob : instance->m_mobList)
                {
                    ...
                    (dynamic_cast<CMobEntity*>(PMob.second))->saveModifiers();
                    (dynamic_cast<CMobEntity*>(PMob.second))->saveMobModifiers();
                }
...
CInstance* CInstanceLoader::LoadInstance(CInstance* instance)
{
    ...
    if (!instance->Failed() && ret != SQL_ERROR /*&& Sql_NumRows(SqlInstanceHandle) != 0*/)
    {
        while (Sql_NextRow(SqlInstanceHandle) == SQL_SUCCESS)
        {
            ...
            (dynamic_cast<CItemWeapon*>(PMob->m_Weapons[SLOT_MAIN]))->setSkillType(Sql_GetIntData(SqlInstanceHandle, 16));
            PMob->m_dmgMult = Sql_GetUIntData(SqlInstanceHandle, 17);
            (dynamic_cast<CItemWeapon*>(PMob->m_Weapons[SLOT_MAIN]))->setDelay((Sql_GetIntData(SqlInstanceHandle, 18) * 1000) / 60);
            (dynamic_cast<CItemWeapon*>(PMob->m_Weapons[SLOT_MAIN]))->setBaseDelay((Sql_GetIntData(SqlInstanceHandle, 18) * 1000) / 60);
