...
int16 CBattleEntity::addTP(int16 tp)
{
    ...
    if (tp > 0)
    {
        ...
        {
            if (dynamic_cast<CPetEntity*>(this)->getPetType() != PETTYPE_AUTOMATON || !this->PMaster)
            {
                                ...
                                DAMAGETYPE damageType /* = DAMAGE_NONE*/)
{
    ...
    {
        roeutils::event(ROE_EVENT::ROE_DMGTAKEN, dynamic_cast<CCharEntity*>(this), RoeDatagram("dmg", amount));
    }
    else if (PLastAttacker && PLastAttacker->objtype == TYPE_PC)
    {
        roeutils::event(ROE_EVENT::ROE_DMGDEALT, dynamic_cast<CCharEntity*>(attacker), RoeDatagram("dmg", amount));
    }
...
uint16 CBattleEntity::ATT()
{
    ...
    else if (this->objtype == TYPE_PET && (dynamic_cast<CPetEntity*>(this))->getPetType() == PETTYPE_AUTOMATON)
    {
...
uint16 CBattleEntity::ACC(uint8 attackNumber, uint8 offsetAccuracy)
{
    ...
    if (this->objtype == TYPE_PET && (dynamic_cast<CPetEntity*>(this))->getPetType() == PETTYPE_AUTOMATON)
    {
...
bool CBattleEntity::ValidTarget(CBattleEntity* PInitiator, uint16 targetFlags)
{
    ...
    return (targetFlags & TARGET_SELF) &&
           (this == PInitiator ||
            (PInitiator->objtype == TYPE_PET && dynamic_cast<CPetEntity*>(PInitiator)->getPetType() == PETTYPE_AUTOMATON && this == PInitiator->PMaster));
}
...
void CBattleEntity::Die()
{
    ...
    {
        dynamic_cast<CBattleEntity*>(PKiller)->ForAlliance([this](CBattleEntity* PMember) {
...
void CBattleEntity::OnCastFinished(CMagicState& state, action_t& action)
{
    ...
    auto*          PActionTarget   = dynamic_cast<CBattleEntity*>(state.GetTarget());
    ...
    else
    {
        if (this->objtype == TYPE_MOB && PActionTarget->objtype == TYPE_PC)
        {
            ...
            IsMagicCovered                   = battleutils::IsMagicCovered(dynamic_cast<CCharEntity*>(PCoverAbilityUser));
    ...
    for (auto* PTarget : PAI->TargetFind->m_targets)
    {
        ...
        if (IsMagicCovered)
        {
            state.ApplyMagicCoverEnmity(POriginalTarget, PActionTarget, dynamic_cast<CMobEntity*>(this));
        }
        else
        {
...
{
    return dynamic_cast<CBattleEntity*>(GetEntity(GetBattleTargetID()));
}
...
bool CBattleEntity::OnAttack(CAttackState& state, action_t& action)
{
    ...
    while (attackRound.GetAttackSwingCount() && !(PTarget->isDead()))
    {
                 ...
                 !PTarget->StatusEffectContainer->HasStatusEffect(EFFECT_ALL_MISS))
        {
                ...
                if (attack.IsCountered())
                {
                    ...
                    else
                    {
                        ...
                        if (PTarget->objtype == TYPE_PC)
                        {
                            ...
                            charutils::TrySkillUP(dynamic_cast<CCharEntity*>(PTarget), (SKILLTYPE)skilltype, GetMLevel());
                        } // In case the Automaton can counter
                        else if (PTarget->objtype == TYPE_PET && PTarget->PMaster && PTarget->PMaster->objtype == TYPE_PC &&
                                 dynamic_cast<CPetEntity*>(PTarget)->getPetType() == PETTYPE_AUTOMATON)
                        {
            ...
            if (PTarget->objtype == TYPE_PC)
            {
                if (attack.IsGuarded() || ((map_config.newstyle_skillups & NEWSTYLE_GUARD) > 0))
                {
                    ...
                    {
                        charutils::TrySkillUP(dynamic_cast<CCharEntity*>(PTarget), SKILL_GUARD, GetMLevel());
                    }
                }
                ...
                if (attack.IsBlocked() || ((map_config.newstyle_skillups & NEWSTYLE_BLOCK) > 0))
                {
                    ...
                    {
                        charutils::TrySkillUP(dynamic_cast<CCharEntity*>(PTarget), SKILL_SHIELD, GetMLevel());
                    }
                }
                ...
                if (attack.IsParried() || ((map_config.newstyle_skillups & NEWSTYLE_PARRY) > 0))
                {
                    ...
                    {
                        charutils::TrySkillUP(dynamic_cast<CCharEntity*>(PTarget), SKILL_PARRY, GetMLevel());
                    }
                }
                if (!attack.IsCountered() && !attack.IsParried())
                {
                    charutils::TrySkillUP(dynamic_cast<CCharEntity*>(PTarget), SKILL_EVASION, GetMLevel());
                }
            }
        }
        else
        ...
        {
            battleutils::HandleEnspell(this, PTarget, &actionTarget, attack.IsFirstSwing(), dynamic_cast<CItemWeapon*>(this->m_Weapons[attack.GetWeaponSlot()]),
                                       attack.GetDamage());
        ...
        if (attack.IsFirstSwing() && attackRound.GetAttackSwingCount() == 1)
        {
            ...
            if (((actionTarget.reaction == REACTION_EVADE || actionTarget.reaction == REACTION_GUARD || actionTarget.spikesEffect == SUBEFFECT_COUNTER) &&
