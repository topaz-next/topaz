...
void CCharEntity::setPetZoningInfo()
{
    ...
    {
        switch ((dynamic_cast<CPetEntity*>(PPet))->getPetType())
        {
            ...
            case PETTYPE_WYVERN:
                ...
                petZoningInfo.petType = (dynamic_cast<CPetEntity*>(PPet))->getPetType();
                break;
            default:
                break;
        }
...
CItemEquipment* CCharEntity::getEquip(SLOTTYPE slot)
{
    ...
    if (loc != 0)
    {
        item = dynamic_cast<CItemEquipment*>(getStorage(est)->GetItem(loc));
    }
    return item;
}
...
{
    auto* controller{ dynamic_cast<CPlayerController*>(PAI->GetController()) };
...
void CCharEntity::OnCastFinished(CMagicState& state, action_t& action)
{
    ...
    for (auto&& actionList : action.actionLists)
    {
        for (auto&& actionTarget : actionList.actionTargets)
        {
            if (actionTarget.param > 0 && PSpell->dealsDamage() && PSpell->getSpellGroup() == SPELLGROUP_BLUE &&
                StatusEffectContainer->HasStatusEffect(EFFECT_CHAIN_AFFINITY) && dynamic_cast<CBlueSpell*>(PSpell)->getPrimarySkillchain() != 0)
            {
                auto*     PBlueSpell = dynamic_cast<CBlueSpell*>(PSpell);
    ...
    if (PSpell->tookEffect())
    {
        ...
        {
            CItemWeapon* PItem = dynamic_cast<CItemWeapon*>(getEquip(SLOT_RANGED));
            ...
            if (primary)
            {
                ...
                if (actionTarget.reaction == REACTION_HIT)
                {
                    if (battleutils::GetScaledItemModifier(this, m_Weapons[damslot], Mod::ADDITIONAL_EFFECT))
                    {
                        ...
                        luautils::OnAdditionalEffect(this, PTarget, dynamic_cast<CItemWeapon*>(m_Weapons[damslot]), &dummy, damage);
                    }
                    ...
                    {
                        actionTarget_t dummy;
                        luautils::OnAdditionalEffect(this, PTarget, dynamic_cast<CItemWeapon*>(getEquip(SLOT_AMMO)), &dummy, damage);
                    }
                    int wspoints = 1;
...
void CCharEntity::OnRangedAttack(CRangeState& state, action_t& action)
{
    ...
    CItemWeapon* PItem = dynamic_cast<CItemWeapon*>(this->getEquip(SLOT_RANGED));
    CItemWeapon* PAmmo = dynamic_cast<CItemWeapon*>(this->getEquip(SLOT_AMMO));
...
CBattleEntity* CCharEntity::IsValidTarget(uint16 targid, uint16 validTargetFlags, std::unique_ptr<CBasicPacket>& errMsg)
{
    ...
    if (PTarget)
    {
        if (PTarget->objtype == TYPE_PC && charutils::IsAidBlocked(this, dynamic_cast<CCharEntity*>(PTarget)))
        {
            // Target is blocking assistance
            errMsg = std::make_unique<CMessageSystemPacket>(0, 0, 225);
            // Interaction was blocked
            dynamic_cast<CCharEntity*>(PTarget)->pushPacket(new CMessageSystemPacket(0, 0, 226));
        }
...
void CCharEntity::UpdateMoghancement()
{
    ...
    for (auto containerID : { LOC_MOGSAFE, LOC_MOGSAFE2 })
    {
        ...
        for (int slotID = 1; slotID <= PContainer->GetSize(); ++slotID)
        {
            ...
            {
                CItemFurnishing* PFurniture = dynamic_cast<CItemFurnishing*>(PItem);
