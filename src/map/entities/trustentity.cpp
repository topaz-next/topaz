...
void CTrustEntity::Die()
{
    ...
    PAI->ClearStateStack();
    PAI->Internal_Die(0s);
    (dynamic_cast<CCharEntity*>(PMaster))->RemoveTrust(this);
    CMobEntity::Die();
}
...
void CTrustEntity::Spawn()
{
    ...
    (dynamic_cast<CCharEntity*>(PMaster))->pushPacket(new CTrustSyncPacket(dynamic_cast<CCharEntity*>(PMaster), this));
}
