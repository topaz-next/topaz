...
bool CAttack::CheckCounter()
{
    ...
    if (m_victim->objtype == TYPE_PC && charutils::hasTrait(dynamic_cast<CCharEntity*>(m_victim), TRAIT_COUNTER))
    {
        if (m_victim->GetMJob() == JOB_MNK || m_victim->GetMJob() == JOB_PUP)
        {
            meritCounter = (dynamic_cast<CCharEntity*>(m_victim))->PMeritPoints->GetMeritValue(MERIT_COUNTER_RATE, dynamic_cast<CCharEntity*>(m_victim));
        }
    }
    ...
    {
        seiganChance = m_victim->getMod(Mod::ZANSHIN) +
                       (dynamic_cast<CCharEntity*>(m_victim))->PMeritPoints->GetMeritValue(MERIT_ZASHIN_ATTACK_RATE, dynamic_cast<CCharEntity*>(m_victim));
...
void CAttack::ProcessDamage()
{
    ...
    {
        m_damage = battleutils::doSoulEaterEffect(dynamic_cast<CCharEntity*>(m_attacker), m_damage);
    }

    // Consume mana
    if (m_attacker->objtype == TYPE_PC)
    {
        m_damage = battleutils::doConsumeManaEffect(dynamic_cast<CCharEntity*>(m_attacker), m_damage);
    }
    ...
    m_damage = attackutils::CheckForDamageMultiplier(dynamic_cast<CCharEntity*>(m_attacker), dynamic_cast<CItemWeapon*>(m_attacker->m_Weapons[slot]), m_damage,
                                                     m_attackType, slot);
    ...
    if (m_damage > 0)
    {
        ...
        {
            charutils::TrySkillUP(dynamic_cast<CCharEntity*>(m_attacker), (SKILLTYPE)weapon->getSkillType(), m_victim->GetMLevel());
        }

        if (m_attacker->objtype == TYPE_PET && m_attacker->PMaster && m_attacker->PMaster->objtype == TYPE_PC &&
            dynamic_cast<CPetEntity*>(m_attacker)->getPetType() == PETTYPE_AUTOMATON)
        {
