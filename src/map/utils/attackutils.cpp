...
uint32 CheckForDamageMultiplier(CCharEntity* PChar, CItemWeapon* PWeapon, uint32 damage, PHYSICAL_ATTACK_TYPE attackType, uint8 weaponSlot)
{
    ...
    if (occ_do_triple_dmg > 0 && tpzrand::GetRandomNumber(100) <= occ_do_triple_dmg)
    {
        return (uint32)(damage * 3.f);
    }
    if (occ_extra_dmg > 2.f && occ_extra_dmg_chance > 0 && tpzrand::GetRandomNumber(100) <= occ_extra_dmg_chance)
    {
        return (uint32)(damage * occ_extra_dmg);
    }
    if (occ_do_double_dmg > 0 && tpzrand::GetRandomNumber(100) <= occ_do_double_dmg)
    {
        return (uint32)(damage * 2.f);
    }
    if (occ_extra_dmg > 0 && occ_extra_dmg_chance > 0 && tpzrand::GetRandomNumber(100) <= occ_extra_dmg_chance)
    {
        return (uint32)(damage * occ_extra_dmg);
    }
