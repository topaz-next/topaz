...
bool isRightRecipe(CCharEntity* PChar)
{
    ...
    int32 ret = Sql_Query(SqlHandle, fmtQuery, PChar->CraftContainer->getItemID(0), PChar->CraftContainer->getItemID(0), PChar->CraftContainer->getItemID(1),
                          PChar->CraftContainer->getItemID(2), PChar->CraftContainer->getItemID(3), PChar->CraftContainer->getItemID(4),
                          PChar->CraftContainer->getItemID(5), PChar->CraftContainer->getItemID(6), PChar->CraftContainer->getItemID(7),
            ...
            if (random < skillUpChance) // If character skills up
            {
                ...
                if ((charSkill + skillUpAmount) > map_config.craft_common_cap) // If character is using the specialization system
                {
                    ...
                    for (uint8 i = SKILL_WOODWORKING; i <= SKILL_COOKING; i++)
                    {
                        ...
                        {
                            skillCumulation += (PChar->RealSkills.skill[i] - map_config.craft_common_cap); // Add to the ammount of specialization points in
                                                                                                           // use.
                            if (skillID != i &&
