    ...
    if (ret != SQL_ERROR && Sql_NumRows(SqlHandle) != 0)
    {
        while (Sql_NextRow(SqlHandle) == SQL_SUCCESS)
        {
            ...
            if (PItem != nullptr)
            {
                ...
                {
                    if (dynamic_cast<CItemLinkshell*>(PItem)->GetLSType() == 0)
                    {
                        dynamic_cast<CItemLinkshell*>(PItem)->SetLSType((LSTYPE)(PItem->getID() - 0x200));
                    }
                ...
                {
                    if ((dynamic_cast<CItemFurnishing*>(PItem))->isInstalled()) // способ узнать, что предмет действительно установлен
                    {
                        PChar->getStorage(LOC_STORAGE)->AddBuff((dynamic_cast<CItemFurnishing*>(PItem))->getStorage());
                    }
                }
            ...
            for (uint8 y = 0; y < MAX_CONTAINER_SIZE; ++y)
            {
                ...
                if (PItem != nullptr && ((PItem->isType(ITEM_EQUIPMENT) || PItem->isType(ITEM_WEAPON)) && !PItem->isSubType(ITEM_CHARGED)))
                {
                    ...
                    for (uint8 j = 0; j < 4; ++j)
                    {
                        ...
                        if ((dynamic_cast<CItemEquipment*>(PItem))->getAugment(j) != 0)
                        {
                            (dynamic_cast<CItemEquipment*>(PItem))->ApplyAugment(j);
                        }
                    }
                }
            }
        ...
        while (Sql_NextRow(SqlHandle) == SQL_SUCCESS)
        {
            ...
            else
            {
                ...
                if ((PItem != nullptr) && PItem->isType(ITEM_LINKSHELL))
                {
                    ...
                    {
                        PLinkshell1 = dynamic_cast<CItemLinkshell*>(PItem);
                    }
                    else if (equipSlot == SLOT_LINK2)
                    {
                        PLinkshell2 = dynamic_cast<CItemLinkshell*>(PItem);
                    }
                }
            }
        }
        ...
        if (Sql_Query(SqlHandle, Query, PChar->id, LocationID, slotID) != SQL_ERROR)
        {
            ...
            if (PChar->getStyleLocked() && !HasItem(PChar, ItemID))
            {
                if (PItem->isType(ITEM_WEAPON))
                {
                    ...
                    {
                        charutils::UpdateWeaponStyle(PChar, SLOT_MAIN, dynamic_cast<CItemWeapon*>(PChar->getEquip(SLOT_MAIN)));
                    }
                    else if (PChar->styleItems[SLOT_SUB] == ItemID)
                    {
                        charutils::UpdateWeaponStyle(PChar, SLOT_SUB, dynamic_cast<CItemWeapon*>(PChar->getEquip(SLOT_SUB)));
                    }
                }
                else if (PItem->isType(ITEM_EQUIPMENT))
                {
                    auto equipSlotID = (dynamic_cast<CItemEquipment*>(PItem))->getSlotType();
...
void UnequipItem(CCharEntity* PChar, uint8 equipSlotID, bool update)
{
    ...
    {
        auto removeSlotID = (dynamic_cast<CItemEquipment*>(PItem))->getRemoveSlotId();
        ...
        if ((dynamic_cast<CItemEquipment*>(PItem))->getScriptType() & SCRIPT_EQUIP)
        {
        ...
        if ((dynamic_cast<CItemEquipment*>(PItem))->getScriptType() & SCRIPT_EQUIP)
        {
            ...
            for (uint8 i = 0; i < 16; ++i)
            {
                ...
                if ((PItem != nullptr) && PItem->isType(ITEM_EQUIPMENT))
                {
                    PChar->m_EquipFlag |= (dynamic_cast<CItemEquipment*>(PItem))->getScriptType();
                }
            }
        }
        ...
        if (PItem->isSubType(ITEM_CHARGED))
        {
            PChar->PRecastContainer->Del(RECAST_ITEM,
                                         PItem->getSlotID() << 8 | PItem->getLocationID()); // Also remove item from the Recast List no matter what bag its in
        }
        ...
        PChar->delEquipModifiers(&(dynamic_cast<CItemEquipment*>(PItem))->modList, (dynamic_cast<CItemEquipment*>(PItem))->getReqLvl(), equipSlotID);
        PChar->PLatentEffectContainer->DelLatentEffects((dynamic_cast<CItemEquipment*>(PItem))->getReqLvl(), equipSlotID);
        PChar->delPetModifiers(&(dynamic_cast<CItemEquipment*>(PItem))->petModList);
        ...
        switch (equipSlotID)
        {
            ...
            case SLOT_RANGED:
            {
                ...
                if ((dynamic_cast<CItemWeapon*>(PItem))->getSkillType() != SKILL_STRING_INSTRUMENT &&
                    (dynamic_cast<CItemWeapon*>(PItem))->getSkillType() != SKILL_WIND_INSTRUMENT)
                {
                    PChar->health.tp = 0;
                }
                PChar->StatusEffectContainer->DelStatusEffect(EFFECT_AFTERMATH);
            ...
            case SLOT_MAIN:
            {
                if (PItem->isType(ITEM_WEAPON))
                {
                    if ((dynamic_cast<CItemWeapon*>(PItem))->getSkillType() == SKILL_HAND_TO_HAND)
                    {
                        PChar->look.sub = 0;
                    }
                }
...
bool EquipArmor(CCharEntity* PChar, uint8 slotID, uint8 equipSlotID, uint8 containerID)
{
    ...
    if (equipSlotID == SLOT_MAIN)
    {
        if (!(slotID == PItem->getSlotID() && oldItem && (oldItem->isType(ITEM_WEAPON) && PItem->isType(ITEM_WEAPON)) &&
              (((dynamic_cast<CItemWeapon*>(PItem))->isTwoHanded()) && ((dynamic_cast<CItemWeapon*>(oldItem))->isTwoHanded()))))
        {
    ...
    if (PItem->getEquipSlotId() & (1 << equipSlotID))
    {
        ...
        switch (equipSlotID)
        {
            case SLOT_MAIN:
            {
                if (PItem->isType(ITEM_WEAPON))
                {
                    switch ((dynamic_cast<CItemWeapon*>(PItem))->getSkillType())
                    {
                        ...
                        case SKILL_STAFF:
                        {
                            ...
                            if ((armor != nullptr) && armor->isType(ITEM_EQUIPMENT))
                            {
                                ...
                                {
                                    CItemWeapon* PWeapon = dynamic_cast<CItemWeapon*>(armor);
                                    if (PWeapon->getSkillType() != SKILL_NONE || (dynamic_cast<CItemWeapon*>(PItem))->getSkillType() == SKILL_HAND_TO_HAND)
                                    {
                    ...
                    if (!(dynamic_cast<CItemWeapon*>(PChar->m_Weapons[SLOT_MAIN]))->isTwoHanded())
                    {
                ...
                UpdateWeaponStyle(PChar, equipSlotID, dynamic_cast<CItemWeapon*>(PItem));
            }
            break;
            case SLOT_SUB:
            {
                ...
                else
                {
                    switch (weapon->getSkillType())
                    {
                        ...
                        {
                            if (PItem->isType(ITEM_WEAPON) &&
                                (!charutils::hasTrait(PChar, TRAIT_DUAL_WIELD) || (dynamic_cast<CItemWeapon*>(PItem))->getSkillType() == SKILL_NONE))
                            {
                                return false;
                            }
                            PChar->m_Weapons[SLOT_SUB] = dynamic_cast<CItemWeapon*>(PItem);
                        ...
                        default:
                        {
                            ...
                            else if (!((dynamic_cast<CItemWeapon*>(PItem))->getSkillType() == SKILL_NONE))
                            {
            ...
            case SLOT_RANGED:
            {
                if (PItem->isType(ITEM_WEAPON))
                {
                    ...
                    if ((weapon != nullptr) && weapon->isType(ITEM_WEAPON))
                    {
                        if ((dynamic_cast<CItemWeapon*>(PItem))->getSkillType() != weapon->getSkillType() ||
                            (dynamic_cast<CItemWeapon*>(PItem))->getSubSkillType() != weapon->getSubSkillType())
                        {
                            UnequipItem(PChar, SLOT_AMMO, false);
                        }
                    }
                    PChar->m_Weapons[SLOT_RANGED] = dynamic_cast<CItemWeapon*>(PItem);
                }
            ...
            case SLOT_AMMO:
            {
                if (PItem->isType(ITEM_WEAPON))
                {
                    ...
                    PChar->m_Weapons[SLOT_AMMO] = dynamic_cast<CItemWeapon*>(PItem);
                    UpdateWeaponStyle(PChar, equipSlotID, dynamic_cast<CItemWeapon*>(PItem));
                }
            }
            break;
...
bool hasValidStyle(CCharEntity* PChar, CItemEquipment* PItem, CItemEquipment* AItem)
{
    return (PItem != nullptr && AItem != nullptr &&
            ((dynamic_cast<CItemWeapon*>(AItem))->getSkillType() == (dynamic_cast<CItemWeapon*>(PItem))->getSkillType()) && HasItem(PChar, AItem->getID()) &&
...
void UpdateWeaponStyle(CCharEntity* PChar, uint8 equipSlotID, CItemWeapon* PItem)
{
    ...
    auto* appearance      = dynamic_cast<CItemEquipment*>(itemutils::GetItem(PChar->styleItems[equipSlotID]));
...
void UpdateArmorStyle(CCharEntity* PChar, uint8 equipSlotID)
{
    ...
    auto* appearance      = dynamic_cast<CItemEquipment*>(itemutils::GetItem(itemID));
...
void EquipItem(CCharEntity* PChar, uint8 slotID, uint8 equipSlotID, uint8 containerID)
{
    ...
    if (equipSlotID == SLOT_SUB && PItem && !PItem->IsShield() && (dynamic_cast<CItemWeapon*>(PItem))->getSkillType() == SKILL_NONE)
    {
        CItemEquipment* PMainItem = PChar->getEquip(SLOT_MAIN);
        if (!PMainItem || !(dynamic_cast<CItemWeapon*>(PMainItem))->isTwoHanded())
        {
    ...
    if (equipSlotID == SLOT_MAIN || equipSlotID == SLOT_RANGED || equipSlotID == SLOT_SUB)
    {
        if (!PItem || !PItem->isType(ITEM_EQUIPMENT) ||
            ((dynamic_cast<CItemWeapon*>(PItem))->getSkillType() != SKILL_STRING_INSTRUMENT &&
             (dynamic_cast<CItemWeapon*>(PItem))->getSkillType() != SKILL_WIND_INSTRUMENT))
        {
...
void CheckEquipLogic(CCharEntity* PChar, SCRIPTTYPE ScriptType, uint32 param)
{
    ...
    for (uint8 slotID = 0; slotID < 16; ++slotID)
    {
        ...
        if ((PItem != nullptr) && PItem->isType(ITEM_EQUIPMENT))
        {
            if ((dynamic_cast<CItemEquipment*>(PItem))->getScriptType() & ScriptType)
            {
...
void DistributeGil(CCharEntity* PChar, CMobEntity* PMob)
{
    ...
    if (PChar->PParty != nullptr)
    {
        ...
        PChar->ForAlliance([PMob, &members](CBattleEntity* PPartyMember) {
            ...
            {
                members.push_back(dynamic_cast<CCharEntity*>(PPartyMember));
            }
        });
...
void SaveFame(CCharEntity* PChar)
{
    ...
    Sql_Query(SqlHandle, Query, PChar->profile.fame[0], PChar->profile.fame[1], PChar->profile.fame[2], PChar->profile.fame[3], PChar->profile.fame[4],
              PChar->profile.fame[5], PChar->profile.fame[6], PChar->profile.fame[7], PChar->profile.fame[8], PChar->profile.fame[9], PChar->profile.fame[10],
              PChar->profile.fame[11], PChar->profile.fame[12], PChar->profile.fame[13], PChar->profile.fame[14], PChar->id);
}
...
void SaveCharEquip(CCharEntity* PChar)
{
    for (uint8 i = 0; i < 18; ++i)
    {
        ...
        {
            const char* fmtQuery = "INSERT INTO char_equip SET charid = %u, equipslotid = %u , slotid  = %u, containerid = %u ON DUPLICATE KEY UPDATE slotid  "
                                   "= %u, containerid = %u;";
...
uint16 AvatarPerpetuationReduction(CCharEntity* PChar)
{
    ...
    uint8 element = (dynamic_cast<CPetEntity*>(PChar->PPet))->m_Element - 1;
...
bool CheckAbilityAddtype(CCharEntity* PChar, CAbility* PAbility)
{
    ...
    if ((PAbility->getAddType() & (ADDTYPE_JUGPET | ADDTYPE_CHARMPET)) == (ADDTYPE_JUGPET | ADDTYPE_CHARMPET))
    {
        if (!PChar->PPet || !(PChar->PPet->objtype == TYPE_MOB ||
                              (PChar->PPet->objtype == TYPE_PET && dynamic_cast<CPetEntity*>(PChar->PPet)->getPetType() == PETTYPE_JUG_PET)))
        {
            return false;
        }
    }
    ...
    {
        if (!PChar->PPet || PChar->PPet->objtype != TYPE_PET || dynamic_cast<CPetEntity*>(PChar->PPet)->getPetType() != PETTYPE_JUG_PET)
        {
            return false;
        }
    }
    ...
    {
        if (!PChar->PPet || PChar->PPet->objtype != TYPE_PET || dynamic_cast<CPetEntity*>(PChar->PPet)->getPetType() != PETTYPE_AVATAR)
        {
            return false;
        }
    }
    ...
    {
        if (!PChar->PPet || PChar->PPet->objtype != TYPE_PET || dynamic_cast<CPetEntity*>(PChar->PPet)->getPetType() != PETTYPE_AUTOMATON)
        {
            return false;
        }
    }
...
void SendToZone(CCharEntity* PChar, uint8 type, uint64 ipp)
{
    if (type == 2)
    {
        Sql_Query(SqlHandle, "UPDATE accounts_sessions SET server_addr = %u, server_port = %u WHERE charid = %u;", (uint32)ipp, (uint32)(ipp >> 32), PChar->id);
...
uint16 getWideScanRange(JOBTYPE job, uint8 level)
{
    ...
    if (job == JOB_BST)
    {
        ...
        if (level >= 20 || map_config.all_jobs_widescan == 1)
        {
            return 150;
        }
