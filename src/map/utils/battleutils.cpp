...
int32 CalculateEnspellDamage(CBattleEntity* PAttacker, CBattleEntity* PDefender, uint8 Tier, uint8 element)
{
    ...
    {
        CItemEquipment* waist = (dynamic_cast<CCharEntity*>(PAttacker))->getEquip(SLOT_WAIST);
...
bool HandleSpikesDamage(CBattleEntity* PAttacker, CBattleEntity* PDefender, actionTarget_t* Action, int32 damage)
{
        ...
        battleutils::GetHitRate(PDefender, PAttacker) / 2 > tpzrand::GetRandomNumber(100) && facing(PDefender->loc.p, PAttacker->loc.p, 64))
    {
        ...
        // Other theories exist but were not proven or reliably tested (I have to assume too many things to even consider JP translations about weapon delay),
        // this at least has data to back it up.
        ...
        else // Struck the target
        {
            if (PDefender->objtype == TYPE_PC)
            {
                ...
                charutils::TrySkillUP(dynamic_cast<CCharEntity*>(PDefender), (SKILLTYPE)skilltype, PAttacker->GetMLevel());
            }
            ...
            dmg = attackutils::CheckForDamageMultiplier((dynamic_cast<CCharEntity*>(PDefender)), dynamic_cast<CItemWeapon*>(PDefender->m_Weapons[SLOT_MAIN]),
    ...
    else if (Action->spikesEffect > 0)
    {
        ...
        if (PDefender->objtype == TYPE_MOB && (dynamic_cast<CMobEntity*>(PDefender))->getMobMod(MOBMOD_AUTO_SPIKES) > 0)
        {
    ...
    else if (PDefender->getMod(Mod::ITEM_SPIKES_TYPE) > 0)
    {
        if (PDefender->objtype == TYPE_PC)
        {
            CCharEntity* PCharDef = dynamic_cast<CCharEntity*>(PDefender);
            ...
            for (auto&& slot : { SLOT_SUB, SLOT_BODY, SLOT_LEGS, SLOT_HEAD, SLOT_HANDS, SLOT_FEET })
            {
                ...
                if (PItem)
                {
                    ...
                    if ((dynamic_cast<CMobEntity*>(PDefender))->m_HiPCLvl < PAttacker->GetMLevel())
                    {
                        (dynamic_cast<CMobEntity*>(PDefender))->m_HiPCLvl = PAttacker->GetMLevel();
                    }
...
void HandleEnspell(CBattleEntity* PAttacker, CBattleEntity* PDefender, actionTarget_t* Action, bool isFirstSwing, CItemWeapon* weapon, int32 finaldamage)
{
    ...
    else if (PAttacker->objtype == TYPE_PC && dynamic_cast<CCharEntity*>(PAttacker)->getEquip(SLOT_SUB) && weapon == PAttacker->m_Weapons[SLOT_MAIN] &&
             dynamic_cast<CItemWeapon*>(dynamic_cast<CCharEntity*>(PAttacker)->getEquip(SLOT_SUB))->getSkillType() == SKILL_NONE &&
             battleutils::GetScaledItemModifier(PAttacker, dynamic_cast<CCharEntity*>(PAttacker)->getEquip(SLOT_SUB), Mod::ADDITIONAL_EFFECT) > 0 &&
             luautils::OnAdditionalEffect(PAttacker, PDefender, dynamic_cast<CItemWeapon*>(dynamic_cast<CCharEntity*>(PAttacker)->getEquip(SLOT_SUB)), Action,
                                          finaldamage) == 0 &&
    ...
    else if (PAttacker->objtype == TYPE_MOB && (dynamic_cast<CMobEntity*>(PAttacker))->getMobMod(MOBMOD_ADD_EFFECT) > 0)
    {
             ...
             PDefender->StatusEffectContainer->HasStatusEffect(EFFECT_ASPIR_DAZE))
    {
        ...
        if (PAttacker->PMaster == nullptr || PAttacker->objtype == TYPE_TRUST)
        {
            ...
            {
                dynamic_cast<CCharEntity*>(PAttacker->PMaster)->ForPartyWithTrusts([&](CBattleEntity* PMember) {
...
uint8 GetRangedHitRate(CBattleEntity* PAttacker, CBattleEntity* PDefender, bool isBarrage, int8 accBonus)
{
    ...
    if (PAttacker->objtype == TYPE_PC)
    {
        ...
        if ((charutils::hasTrait(dynamic_cast<CCharEntity*>(PAttacker), TRAIT_AMBUSH)) && behind(PAttacker->loc.p, PDefender->loc.p, 64))
        {
            acc += (dynamic_cast<CCharEntity*>(PAttacker))->PMeritPoints->GetMeritValue(MERIT_AMBUSH, dynamic_cast<CCharEntity*>(PAttacker));
        }
    }
    else if (PAttacker->objtype == TYPE_PET && (dynamic_cast<CPetEntity*>(PAttacker))->getPetType() == PETTYPE_AUTOMATON)
    {
...
bool TryInterruptSpell(CBattleEntity* PAttacker, CBattleEntity* PDefender, CSpell* PSpell)
{
    ...
    { // Check player's skill.
        ...
        CCharEntity* PChar = dynamic_cast<CCharEntity*>(PDefender);
        ...
        meritReduction =
            (dynamic_cast<CCharEntity*>(PDefender))->PMeritPoints->GetMeritValue(MERIT_SPELL_INTERUPTION_RATE, dynamic_cast<CCharEntity*>(PDefender));
    }
...
uint8 GetBlockRate(CBattleEntity* PAttacker, CBattleEntity* PDefender)
{
    ...
    {
        CMobEntity* PMob = dynamic_cast<CMobEntity*>(PDefender);
        if (PMob->m_EcoSystem != SYSTEM_UNDEAD && PMob->m_EcoSystem != SYSTEM_BEASTMEN)
        {
            return 0;
        }
    }
    else if (PDefender->objtype == TYPE_PET && dynamic_cast<CPetEntity*>(PDefender)->getPetType() == PETTYPE_AUTOMATON && PDefender->GetMJob() == JOB_PLD)
    {
...
uint8 GetParryRate(CBattleEntity* PAttacker, CBattleEntity* PDefender)
{
        ...
        PDefender->PAI->IsEngaged())
    {
            ...
            job == JOB_GEO || job == JOB_SCH)
        {
            ...
            // Issekigan grants parry rate bonus. From best available data, if you already capped out at 25% parry it grants another 25% bonus for ~50% parry
            // rate
...
int32 TakePhysicalDamage(CBattleEntity* PAttacker, CBattleEntity* PDefender, PHYSICAL_ATTACK_TYPE physicalAttackType, int32 damage, bool isBlocked, uint8 slot,
                         uint16 tpMultiplier, CBattleEntity* taChar, bool giveTPtoVictim, bool giveTPtoAttacker, bool isCounter, bool isCovered,
                         CBattleEntity* POriginalTarget)
{
    ...
    if (PAttacker->StatusEffectContainer->HasStatusEffect(EFFECT_FORMLESS_STRIKES) && !isCounter)
    {
        ...
        if (PAttacker->objtype == TYPE_PC)
        {
            formlessMod += (dynamic_cast<CCharEntity*>(PAttacker))->PMeritPoints->GetMeritValue(MERIT_FORMLESS_STRIKES, dynamic_cast<CCharEntity*>(PAttacker));
        }
    ...
    else
    {
        ...
        if (!isCounter || giveTPtoAttacker) // counters are always considered blunt (assuming h2h) damage, except retaliation (which is the only counter that
                                            // gives TP to the attacker)
        {
            switch (damageType)
            {
            ...
            if (PDefender->m_Weapons[SLOT_SUB]->IsShield())
            {
                if (PDefender->objtype == TYPE_PC)
                {
                    ...
                    if ((std::max(damage - (PDefender->getMod(Mod::PHALANX) + PDefender->getMod(Mod::STONESKIN)), 0) > 0) &&
                        charutils::hasTrait(dynamic_cast<CCharEntity*>(PDefender), TRAIT_SHIELD_MASTERY))
                    {
    ...
    if (damage > 0)
    {
        ...
        switch (PDefender->objtype)
        {
            case TYPE_MOB:
                if (taChar == nullptr)
                {
                    (dynamic_cast<CMobEntity*>(PDefender))->PEnmityContainer->UpdateEnmityFromDamage(PAttacker, damage);
                }
                else
                {
                    (dynamic_cast<CMobEntity*>(PDefender))->PEnmityContainer->UpdateEnmityFromDamage(taChar, damage);
                }
                ...
                {
                    (dynamic_cast<CPetEntity*>(PDefender))
            ...
            case TYPE_PC:
                if (PAttacker->objtype == TYPE_MOB)
                {
                    if (isCovered)
                    {
                        (dynamic_cast<CMobEntity*>(PAttacker))->PEnmityContainer->UpdateEnmityFromCover(POriginalTarget, PDefender);
                    }
                    else
                    {
                        (dynamic_cast<CMobEntity*>(PAttacker))->PEnmityContainer->UpdateEnmityFromAttack(PDefender, damage);
                    }
                }
                break;
            default:
        ...
        if ((!isRanged) &&
            !((isBlocked) && (PDefender->objtype == TYPE_PC) && (charutils::hasTrait(dynamic_cast<CCharEntity*>(PDefender), TRAIT_SHIELD_MASTERY))))
        {
        ...
        if (giveTPtoAttacker)
        {
            ...
            {
                baseTp += (dynamic_cast<CCharEntity*>(PAttacker))->PMeritPoints->GetMeritValue(MERIT_IKISHOTEN, dynamic_cast<CCharEntity*>(PAttacker));
            }
        ...
        if (giveTPtoVictim)
        {
            ...
            {
                PDefender->addTP(
                    (uint16)(tpMultiplier * ((baseTp + 30) * sBlowMult *
    ...
    {
        (dynamic_cast<CMobEntity*>(PDefender))->PEnmityContainer->UpdateEnmityFromDamage(PAttacker, 0);
    }
                            ...
                            bool primary, float tpMultiplier, uint16 bonusTP, float targetTPMultiplier)
{
    ...
    if (damage > 0)
    {
        ...
        if (PDefender->objtype == TYPE_PC)
        {
            PDefender->addTP((int16)(
                tpMultiplier * targetTPMultiplier *
                ((baseTp / 3) * sBlowMult *
                 (1.0f + 0.01f * (float)((PDefender->getMod(Mod::STORETP) + getStoreTPbonusFromMerit(PAttacker))))))); // yup store tp counts on hits taken too!
        }
        else
        {
            PDefender->addTP((int16)(
                tpMultiplier * targetTPMultiplier *
                ((baseTp + 30) * sBlowMult * (1.0f + 0.01f * (float)PDefender->getMod(Mod::STORETP))))); // subtle blow also reduces the "+30" on mob tp gain
        }
    }
    else if (PDefender->objtype == TYPE_MOB)
    {
                   ...
                   int8 offsetAccuracy) // subWeaponAttack is for calculating acc of dual wielded sub weapon
{
    ...
    if (PAttacker->objtype == TYPE_PC &&
        ((PAttacker->StatusEffectContainer->HasStatusEffect(EFFECT_SNEAK_ATTACK) &&
         ...
         (charutils::hasTrait(dynamic_cast<CCharEntity*>(PAttacker), TRAIT_ASSASSIN) &&
    ...
    else
    {
        ...
        if (PAttacker->objtype == TYPE_PC && (charutils::hasTrait(dynamic_cast<CCharEntity*>(PAttacker), TRAIT_AMBUSH)) &&
            behind(PAttacker->loc.p, PDefender->loc.p, 64))
        {
            offsetAccuracy += (dynamic_cast<CCharEntity*>(PAttacker))->PMeritPoints->GetMeritValue(MERIT_AMBUSH, dynamic_cast<CCharEntity*>(PAttacker));
        }
        // Check for Closed Position merit on attacker for additional accuracy and that attacker and defender are facing each other
        if (PAttacker->objtype == TYPE_PC && (charutils::hasTrait(dynamic_cast<CCharEntity*>(PAttacker), TRAIT_CLOSED_POSITION)) &&
            (infront(PAttacker->loc.p, PDefender->loc.p, 64) && facing(PAttacker->loc.p, PDefender->loc.p, 64)))
        {
            offsetAccuracy +=
                (dynamic_cast<CCharEntity*>(PAttacker))->PMeritPoints->GetMeritValue(MERIT_CLOSED_POSITION, dynamic_cast<CCharEntity*>(PAttacker));
        }
        // Check for Closed Position merit on defender for additional evasion and that attacker and defender are facing each other
        if (PDefender->objtype == TYPE_PC && (charutils::hasTrait(dynamic_cast<CCharEntity*>(PDefender), TRAIT_CLOSED_POSITION)) &&
            (infront(PDefender->loc.p, PAttacker->loc.p, 64) && facing(PDefender->loc.p, PAttacker->loc.p, 64)))
        {
            offsetAccuracy -=
                (dynamic_cast<CCharEntity*>(PDefender))->PMeritPoints->GetMeritValue(MERIT_CLOSED_POSITION, dynamic_cast<CCharEntity*>(PDefender));
        }
...
uint8 GetCritHitRate(CBattleEntity* PAttacker, CBattleEntity* PDefender, bool ignoreSneakTrickAttack)
{
    ...
    if (PAttacker->objtype == TYPE_PC && (!ignoreSneakTrickAttack) && PAttacker->StatusEffectContainer->HasStatusEffect(EFFECT_SNEAK_ATTACK))
    {
    ...
    else if (PAttacker->objtype == TYPE_PC && PAttacker->GetMJob() == JOB_THF && charutils::hasTrait(dynamic_cast<CCharEntity*>(PAttacker), TRAIT_ASSASSIN) &&
    ...
    else
    {
        ...
        if (PAttacker->objtype == TYPE_PC)
        {
            CCharEntity* PCharAttacker = dynamic_cast<CCharEntity*>(PAttacker);
        ...
        if (PDefender->objtype == TYPE_PC)
        {
            crithitrate -= (dynamic_cast<CCharEntity*>(PDefender))->PMeritPoints->GetMeritValue(MERIT_ENEMY_CRIT_RATE, dynamic_cast<CCharEntity*>(PDefender));
        }
...
uint8 CheckMultiHits(CBattleEntity* PEntity, CItemWeapon* PWeapon)
{
    ...
    if (PEntity->objtype == TYPE_PC)
    {
        ...
        {
            tripleAttack += PChar->PMeritPoints->GetMeritValue(MERIT_TRIPLE_ATTACK_RATE, dynamic_cast<CCharEntity*>(PEntity));
        }
        if (charutils::hasTrait(PChar, TRAIT_DOUBLE_ATTACK))
        {
            doubleAttack += PChar->PMeritPoints->GetMeritValue(MERIT_DOUBLE_ATTACK_RATE, dynamic_cast<CCharEntity*>(PEntity));
        }
    }
    ...
    if (PEntity->GetMJob() == JOB_SAM)
    {
        if (PEntity->StatusEffectContainer->HasStatusEffect(EFFECT_HASSO))
        {
            ...
            if (PEntity->objtype == TYPE_PC)
            {
                zanshin += (dynamic_cast<CCharEntity*>(PEntity))->PMeritPoints->GetMeritValue(MERIT_ZASHIN_ATTACK_RATE, dynamic_cast<CCharEntity*>(PEntity));
            }
...
int32 TakeSkillchainDamage(CBattleEntity* PAttacker, CBattleEntity* PDefender, int32 lastSkillDamage, CBattleEntity* taChar)
{
    ...
    switch (PDefender->objtype)
    {
        ...
        case TYPE_MOB:
        {
            (dynamic_cast<CMobEntity*>(PDefender))->PEnmityContainer->UpdateEnmityFromDamage(taChar ? taChar : PAttacker, (uint16)damage);
        }
        break;
        default:
        {
...
CItemEquipment* GetEntityArmor(CBattleEntity* PEntity, SLOTTYPE Slot)
{
    ...
    if (PEntity->objtype == TYPE_PC)
    {
        return ((dynamic_cast<CCharEntity*>(PEntity))->getEquip(Slot));
    }
    return nullptr;
}
...
CItemWeapon* GetEntityWeapon(CBattleEntity* PEntity, SLOTTYPE Slot)
{
    ...
    return dynamic_cast<CItemWeapon*>((dynamic_cast<CMobEntity*>(PEntity))->m_Weapons[Slot]);
}
...
void MakeEntityStandUp(CBattleEntity* PEntity)
{
    ...
    if (PEntity->objtype == TYPE_PC)
    {
        CCharEntity* PPlayer = (dynamic_cast<CCharEntity*>(PEntity));
...
void GenerateInRangeEnmity(CBattleEntity* PSource, int16 CE, int16 VE)
{
    ...
    {
        PIterSource = dynamic_cast<CCharEntity*>(PSource);
    }
...
uint8 getStoreTPbonusFromMerit(CBattleEntity* PEntity)
{
    if (PEntity->objtype == TYPE_PC)
    {
        if ((dynamic_cast<CCharEntity*>(PEntity))->GetMJob() == JOB_SAM)
        {
            return (dynamic_cast<CCharEntity*>(PEntity))->PMeritPoints->GetMeritValue(MERIT_STORE_TP_EFFECT, dynamic_cast<CCharEntity*>(PEntity));
        }
    }
    return 0;
}
...
uint16 jumpAbility(CBattleEntity* PAttacker, CBattleEntity* PVictim, uint8 tier)
{
    ...
    {
        (dynamic_cast<CMobEntity*>(PVictim))->PEnmityContainer->LowerEnmityByPercent(PAttacker, 99, nullptr);
        return 0;
    }
    ...
    for (uint8 i = 0; i < (numattacksLeftHand + numattacksRightHand); ++i)
    {
        ...
        if (tpzrand::GetRandomNumber(100) < hitrate)
        {
            ...
            if (!battleutils::IsAbsorbByShadow(PVictim))
            {
                ...
                damageForRound = attackutils::CheckForDamageMultiplier(dynamic_cast<CCharEntity*>(PAttacker), PWeapon, damageForRound,
    ...
    if (PAttacker->objtype == TYPE_PC)
    {
        totalDamage = battleutils::doSoulEaterEffect(dynamic_cast<CCharEntity*>(PAttacker), totalDamage);
    }
    ...
    if (tier == 2 && PVictim->objtype == TYPE_MOB)
    {
        ...
        (dynamic_cast<CMobEntity*>(PVictim))->PEnmityContainer->LowerEnmityByPercent(PAttacker, (uint8)enmityReduction, nullptr);
    }
    ...
    if (PAttacker->objtype == TYPE_PC)
    {
        charutils::TrySkillUP(dynamic_cast<CCharEntity*>(PAttacker), (SKILLTYPE)PWeapon->getSkillType(), PVictim->GetMLevel());
    }
...
void tryToCharm(CBattleEntity* PCharmer, CBattleEntity* PVictim)
{
    ...
    if (PMob && PCharmer->objtype == TYPE_PC)
    {
        ...
        if ((dynamic_cast<CMobEntity*>(PVictim))->getMobMod(MOBMOD_CHARMABLE) == 0 || PVictim->PMaster != nullptr)
        {
...
void applyCharm(CBattleEntity* PCharmer, CBattleEntity* PVictim, duration charmTime)
{
    ...
    if (PVictim->objtype == TYPE_MOB)
    {
        ...
        (dynamic_cast<CMobEntity*>(PVictim))->PEnmityContainer->Clear();

        // set the mobs ai to petAi
        PCharmer->PPet->PAI->SetController(std::make_unique<CPetController>(dynamic_cast<CPetEntity*>(PCharmer->PPet)));
        ...
        charutils::BuildingCharAbilityTable(dynamic_cast<CCharEntity*>(PCharmer));
        charutils::BuildingCharPetAbilityTable(dynamic_cast<CCharEntity*>(PCharmer), dynamic_cast<CPetEntity*>(PVictim), PVictim->id);
        (dynamic_cast<CCharEntity*>(PCharmer))->pushPacket(new CCharUpdatePacket(dynamic_cast<CCharEntity*>(PCharmer)));
        (dynamic_cast<CCharEntity*>(PCharmer))->pushPacket(new CPetSyncPacket(dynamic_cast<CCharEntity*>(PCharmer)));
        PCharmer->ForAlliance([&PVictim](CBattleEntity* PMember) {
            if (dynamic_cast<CCharEntity*>(PMember)->PClaimedMob == PVictim)
            {
                dynamic_cast<CCharEntity*>(PMember)->PClaimedMob = nullptr;
            }
        });
        (dynamic_cast<CMobEntity*>(PVictim))->m_OwnerID.clean();
    ...
    else if (PVictim->objtype == TYPE_PC)
    {
        ...
        dynamic_cast<CCharEntity*>(PVictim)->ClearTrusts();

        PVictim->PAI->SetController(std::make_unique<CPlayerCharmController>(dynamic_cast<CCharEntity*>(PVictim)));

        battleutils::RelinquishClaim(dynamic_cast<CCharEntity*>(PVictim));
        PVictim->PMaster = PCharmer;
...
void unCharm(CBattleEntity* PEntity)
{
    if (PEntity->objtype == TYPE_PC)
    {
        ...
        PEntity->PAI->SetController(std::make_unique<CPlayerController>(dynamic_cast<CCharEntity*>(PEntity)));
...
void ClaimMob(CBattleEntity* PDefender, CBattleEntity* PAttacker, bool passing)
{
    ...
    if (PDefender->objtype == TYPE_MOB)
    {
        ...
        CMobEntity*    mob          = dynamic_cast<CMobEntity*>(PDefender);
        if (!passing)
        {
        ...
        if (PAttacker)
        {
            CCharEntity* attacker = dynamic_cast<CCharEntity*>(PAttacker);
            if (!passing)
            {
            ...
            if (!battleTarget || battleTarget == PDefender || battleTarget != attacker->PClaimedMob || PDefender->isDead())
            {
                ...
                { // unclaim any other living mobs owned by attacker
                    dynamic_cast<CMobController*>(attacker->PClaimedMob->PAI->GetController())->TapDeclaimTime();
                ...
                if (!mob->CalledForHelp())
                {
                    ...
                    { // mob is unclaimed
                        ...
                        {
                            highestClaim = dynamic_cast<CTrustEntity*>(highestClaim)->PMaster;
                        }
...
void RelinquishClaim(CCharEntity* PChar)
{
    ...
    { // if we currently own a mob
        ...
        static_cast<CBattleEntity*>(PChar)->ForAlliance([&PChar, &mob, &found](CBattleEntity* PMember) {
            CCharEntity* member = dynamic_cast<CCharEntity*>(PMember);
        ...
        { // if mob didn't pass to someone else, unclaim it
            dynamic_cast<CMobController*>(mob->PAI->GetController())->TapDeclaimTime();
        }
    }
...
void HandleIssekiganEnmityBonus(CBattleEntity* PDefender, CBattleEntity* PAttacker)
{
    if (PAttacker->objtype == TYPE_MOB && PDefender->StatusEffectContainer->HasStatusEffect(EFFECT_ISSEKIGAN))
    {
        ...
        dynamic_cast<CMobEntity*>(PAttacker)->PEnmityContainer->UpdateEnmity(PDefender, 300, 0, false);
    }
}
...
float HandleTranquilHeart(CBattleEntity* PEntity)
{
    ...
    if (PEntity->objtype == TYPE_PC && charutils::hasTrait(dynamic_cast<CCharEntity*>(PEntity), TRAIT_TRANQUIL_HEART))
    {
...
int32 HandleSevereDamage(CBattleEntity* PDefender, int32 damage, bool isPhysical)
{
    ...
    if (isPhysical && PDefender->objtype == TYPE_PET && PDefender->getMod(Mod::AUTO_SCHURZEN) != 0 && damage >= PDefender->health.hp &&
        (dynamic_cast<CPetEntity*>(PDefender))->PMaster->StatusEffectContainer->GetEffectsCount(EFFECT_EARTH_MANEUVER) >= 1)
    {
        damage = PDefender->health.hp - 1;
        (dynamic_cast<CPetEntity*>(PDefender))->PMaster->StatusEffectContainer->DelStatusEffectSilent(EFFECT_EARTH_MANEUVER);
    }
...
void assistTarget(CCharEntity* PChar, uint16 TargID)
{
    ...
    CBattleEntity* PlayerToAssist = dynamic_cast<CBattleEntity*>(PChar->GetEntity(TargID, TYPE_MOB | TYPE_PC));
    if (PlayerToAssist != nullptr)
    {
        if (PlayerToAssist->objtype == TYPE_PC && PlayerToAssist->m_TargID != 0)
        {
            ...
            CBattleEntity* EntityToLockon = dynamic_cast<CBattleEntity*>(PChar->GetEntity(PlayerToAssist->m_TargID, TYPE_MOB | TYPE_PC | TYPE_PET));
...
uint8 GetSpellAoEType(CBattleEntity* PCaster, CSpell* PSpell)
{
    if (PSpell->getAOE() == SPELLAOE_RADIAL_ACCE) // Divine Veil goes here because -na spells have AoE w/ Accession
    {
        if (PCaster->StatusEffectContainer->HasStatusEffect(EFFECT_ACCESSION) ||
            (PCaster->objtype == TYPE_PC && charutils::hasTrait(dynamic_cast<CCharEntity*>(PCaster), TRAIT_DIVINE_VEIL) && PSpell->isNa() &&
    ...
    std::function<void(CBattleEntity*)> drawInFunc = [PMob, drawInDistance, &nearEntity, &success](CBattleEntity* PMember) {
        ...
        if (PMob->loc.zone == PMember->loc.zone && pDistance > drawInDistance && PMember->status != STATUS_CUTSCENE_ONLY)
        {
            ...
            else
            {
                ...
                {
                    CCharEntity* PChar = dynamic_cast<CCharEntity*>(PMember);
...
{
    CCharEntity* PChar = PEntity->objtype == TYPE_PC ? dynamic_cast<CCharEntity*>(PEntity) : nullptr;
                ...
                if (PExistingTrait->getID() == PTrait->getID())
                {
                    ...
                    if (PChar)
                    {
                        if (PExistingTrait->getMeritID() > 0)
                        {
                            ...
                            if (PExistingTrait->getMeritID() == PTrait->getMeritID())
                            {
                                add = false;
                                break;
                            }
...
uint32 CalculateSpellCastTime(CBattleEntity* PEntity, CMagicState* PMagicState)
{
    ...
    else if (PSpell->getSpellGroup() == SPELLGROUP_SONG)
    {
        ...
        {
            if (PEntity->objtype == TYPE_PC &&
                tpzrand::GetRandomNumber(100) <
                    (dynamic_cast<CCharEntity*>(PEntity))->PMeritPoints->GetMeritValue(MERIT_NIGHTINGALE, dynamic_cast<CCharEntity*>(PEntity)) - 25)
            {
                return 0;
            }
    ...
    else if (PSpell->isCure()) // Cure cast time reductions
    {
        ...
        if (PEntity->objtype == TYPE_PC)
        {
            fastCast += (dynamic_cast<CCharEntity*>(PEntity))->PMeritPoints->GetMeritValue(MERIT_CURE_CAST_TIME, dynamic_cast<CCharEntity*>(PEntity));
        }
    }
...
uint32 CalculateSpellRecastTime(CBattleEntity* PEntity, CSpell* PSpell)
{
    ...
    if (PSpell->getSpellGroup() == SPELLGROUP_SONG)
    {
        ...
        if (PEntity->objtype == TYPE_PC)
        {
            ...
            {
                recast -= (dynamic_cast<CCharEntity*>(PEntity))->PMeritPoints->GetMeritValue(MERIT_FINALE_RECAST, dynamic_cast<CCharEntity*>(PEntity)) * 1000;
            }
            ...
            {
                recast -= (dynamic_cast<CCharEntity*>(PEntity))->PMeritPoints->GetMeritValue(MERIT_LULLABY_RECAST, dynamic_cast<CCharEntity*>(PEntity)) * 1000;
            }
        }
...
{
    CItemWeapon* PItem = dynamic_cast<CItemWeapon*>(PChar->getEquip(SLOT_AMMO));
...
int32 GetMeritValue(CBattleEntity* PEntity, MERIT_TYPE merit)
{
    if (PEntity->objtype == TYPE_PC)
    {
        return dynamic_cast<CCharEntity*>(PEntity)->PMeritPoints->GetMeritValue(merit, dynamic_cast<CCharEntity*>(PEntity));
    }
    return 0;
}
