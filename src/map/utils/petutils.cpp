...
void LoadJugStats(CPetEntity* PMob, Pet_t* petStats)
{
    ...
    (dynamic_cast<CItemWeapon*>(PMob->m_Weapons[SLOT_MAIN]))->setDamage(GetJugWeaponDamage(PMob));

    // reduce weapon delay of MNK
    if (PMob->GetMJob() == JOB_MNK)
    {
        (dynamic_cast<CItemWeapon*>(PMob->m_Weapons[SLOT_MAIN]))->resetDelay();
    }
...
void LoadAutomatonStats(CCharEntity* PMaster, CPetEntity* PPet, Pet_t* petStats)
{
    ...
    (dynamic_cast<CItemWeapon*>(PPet->m_Weapons[SLOT_MAIN]))->setSkillType(SKILL_AUTOMATON_MELEE);
    (dynamic_cast<CItemWeapon*>(PPet->m_Weapons[SLOT_MAIN]))
        ->setDelay((uint16)(floor(1000.0f * (petStats->cmbDelay / 60.0f)))); // every pet should use this eventually
    (dynamic_cast<CItemWeapon*>(PPet->m_Weapons[SLOT_MAIN]))->setDamage((PPet->GetSkill(SKILL_AUTOMATON_MELEE) / 9) * 2 + 3);

    (dynamic_cast<CItemWeapon*>(PPet->m_Weapons[SLOT_RANGED]))->setSkillType(SKILL_AUTOMATON_RANGED);
    (dynamic_cast<CItemWeapon*>(PPet->m_Weapons[SLOT_RANGED]))->setDamage((PPet->GetSkill(SKILL_AUTOMATON_RANGED) / 9) * 2 + 3);
...
void SpawnPet(CBattleEntity* PMaster, uint32 PetID, bool spawningFromZone)
{
    ...
    {
        puppetutils::LoadAutomaton(dynamic_cast<CCharEntity*>(PMaster));
        PMaster->PPet = dynamic_cast<CCharEntity*>(PMaster)->PAutomaton;
    }
    else
    {
    ...
    if (PPet)
    {
        ...
        if (PMaster->objtype == TYPE_PC)
        {
            charutils::BuildingCharAbilityTable(dynamic_cast<CCharEntity*>(PMaster));
            charutils::BuildingCharPetAbilityTable(dynamic_cast<CCharEntity*>(PMaster), PPet, PetID);
            (dynamic_cast<CCharEntity*>(PMaster))->pushPacket(new CCharUpdatePacket(dynamic_cast<CCharEntity*>(PMaster)));
            ...
            (dynamic_cast<CCharEntity*>(PMaster))->PLatentEffectContainer->CheckLatentsPetType();
            PMaster->ForParty([](CBattleEntity* PMember) { (dynamic_cast<CCharEntity*>(PMember))->PLatentEffectContainer->CheckLatentsPartyAvatar(); });
        }
        // apply stats from previous zone if this pet is being transfered
        if (spawningFromZone)
        {
            PPet->health.tp = (int16)(dynamic_cast<CCharEntity*>(PMaster))->petZoningInfo.petTP;
            PPet->health.hp = (dynamic_cast<CCharEntity*>(PMaster))->petZoningInfo.petHP;
            PPet->health.mp = (dynamic_cast<CCharEntity*>(PMaster))->petZoningInfo.petMP;
        }
    }
    else if (PMaster->objtype == TYPE_PC)
    {
        dynamic_cast<CCharEntity*>(PMaster)->resetPetZoningInfo();
    }
}
...
void DetachPet(CBattleEntity* PMaster)
{
    ...
    {
        CMobEntity* PMob = dynamic_cast<CMobEntity*>(PPet);
    ...
    else if (PPet->objtype == TYPE_PET)
    {
        ...
        CPetEntity* PPetEnt = dynamic_cast<CPetEntity*>(PPet);
...
void LoadPet(CBattleEntity* PMaster, uint32 PetID, bool spawningFromZone)
{
    ...
    if (PMaster->objtype == TYPE_PC)
    {
        (dynamic_cast<CCharEntity*>(PMaster))->petZoningInfo.petID = PetID;
    }
    ...
    */
    ...
    {
        PPet = (dynamic_cast<CCharEntity*>(PMaster))->PAutomaton;
        PPet->PAI->SetController(std::make_unique<CAutomatonController>(dynamic_cast<CAutomatonEntity*>(PPet)));
    }
    else
    {
    ...
    if (PPet->getPetType() == PETTYPE_AVATAR)
    {
        ...
        (dynamic_cast<CItemWeapon*>(PPet->m_Weapons[SLOT_MAIN]))->setDelay((uint16)(floor(1000.0f * (320.0f / 60.0f))));

        if (PetID == PETID_FENRIR)
        {
            (dynamic_cast<CItemWeapon*>(PPet->m_Weapons[SLOT_MAIN]))->setDelay((uint16)(floor(1000.0 * (280.0f / 60.0f))));
        }
        ...
        (dynamic_cast<CItemWeapon*>(PPet->m_Weapons[SLOT_MAIN]))->setDamage(weaponDamage);
    ...
    {
        (dynamic_cast<CItemWeapon*>(PPet->m_Weapons[SLOT_MAIN]))->setDelay((uint16)(floor(1000.0f * (240.0f / 60.0f))));
    ...
    else if (PPet->getPetType() == PETTYPE_AUTOMATON && PMaster->objtype == TYPE_PC)
    {
        ...
        LoadAutomatonStats(dynamic_cast<CCharEntity*>(PMaster), PPet, g_PPetList.at(PetID)); // temp
        if (PMaster->objtype == TYPE_PC)
        {
            CCharEntity* PChar = dynamic_cast<CCharEntity*>(PMaster);
...
void LoadWyvernStatistics(CBattleEntity* PMaster, CPetEntity* PPet, bool finalize)
{
    ...
    LoadAvatarStats(PPet);                                                                                           // follows PC calcs (w/o SJ)
    (dynamic_cast<CItemWeapon*>(PPet->m_Weapons[SLOT_MAIN]))->setDelay((uint16)(floor(1000.0f * (320.0f / 60.0f)))); // 320 delay
    (dynamic_cast<CItemWeapon*>(PPet->m_Weapons[SLOT_MAIN]))->setDamage((uint16)(1 + floor(PPet->GetMLevel() * 0.9f)));
    // Set A+ weapon skill
...
bool CheckPetModType(CBattleEntity* PPet, PetModType petmod)
{
    ...
    if (auto* PPetEntity = dynamic_cast<CPetEntity*>(PPet))
    {
        ...
        {
            if (petmod == PetModType::Automaton || (uint16)petmod + 28 == (uint16) dynamic_cast<CAutomatonEntity*>(PPetEntity)->getFrame())
            {
                return true;
            }
        }
