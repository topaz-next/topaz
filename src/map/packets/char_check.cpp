    ...
    for (int32 i = 0; i < 16; ++i)
    {
        ...
        if (PItem != nullptr)
        {
            ...
            if (PItem->isSubType(ITEM_CHARGED))
            {
                ...
                uint32 nextUseTime = (dynamic_cast<CItemUsable*>(PItem))->getLastUseTime() + (dynamic_cast<CItemUsable*>(PItem))->getReuseDelay();

                ref<uint8>(size * 2 + 0x04) = 0x01;
                ref<uint8>(size * 2 + 0x05) = (dynamic_cast<CItemUsable*>(PItem))->getCurrentCharges();
                ref<uint8>(size * 2 + 0x07) = (nextUseTime > currentTime ? 0x90 : 0xD0);

                ref<uint32>(size * 2 + 0x08) = nextUseTime;
                ref<uint32>(size * 2 + 0x0C) = (dynamic_cast<CItemUsable*>(PItem))->getUseDelay() + currentTime;
            }
            ...
            if (PItem->isSubType(ITEM_AUGMENTED))
            {
                ...
                ref<uint16>(size * 2 + 0x06) = (dynamic_cast<CItemEquipment*>(PItem))->getAugment(0);
                ref<uint16>(size * 2 + 0x08) = (dynamic_cast<CItemEquipment*>(PItem))->getAugment(1);
                ref<uint16>(size * 2 + 0x0A) = (dynamic_cast<CItemEquipment*>(PItem))->getAugment(2);
                ref<uint16>(size * 2 + 0x0C) = (dynamic_cast<CItemEquipment*>(PItem))->getAugment(3);
            }
