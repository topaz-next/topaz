...
CTradeUpdatePacket::CTradeUpdatePacket(CItem* PItem, uint8 SlotID)
{
    ...
    if (PItem->isSubType(ITEM_CHARGED))
    {
        ...
        {
            ref<uint8>(0x0F) = (dynamic_cast<CItemUsable*>(PItem))->getCurrentCharges();
        }
    }
    if (PItem->isType(ITEM_LINKSHELL))
    {
        ref<uint32>(0x0E) = (dynamic_cast<CItemLinkshell*>(PItem))->GetLSID();
        ref<uint16>(0x14) = (dynamic_cast<CItemLinkshell*>(PItem))->GetLSRawColor();
        ref<uint8>(0x16)  = (dynamic_cast<CItemLinkshell*>(PItem))->GetLSType();
