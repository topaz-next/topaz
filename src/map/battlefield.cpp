...
bool CBattlefield::InsertEntity(CBaseEntity* PEntity, bool enter, BATTLEFIELDMOBCONDITION conditions, bool ally)
{
    ...
    else if (PEntity->objtype == TYPE_NPC)
    {
        ...
        m_NpcList.push_back(dynamic_cast<CNpcEntity*>(PEntity));
    }
    else if (PEntity->objtype == TYPE_MOB || PEntity->objtype == TYPE_PET)
    {
        ...
        if (!ally)
        {
            ...
            else
            {
                ...
                mob.PMob                  = dynamic_cast<CMobEntity*>(PEntity);
        ...
        {
            m_AllyList.push_back(dynamic_cast<CMobEntity*>(PEntity));
        }
    }
...
CBaseEntity* CBattlefield::GetEntity(CBaseEntity* PEntity)
{
    ...
    {
        if (auto* POwner = dynamic_cast<CCharEntity*>(dynamic_cast<CBattleEntity*>(PEntity)->PMaster))
        {
            for (const auto id : m_EnteredPlayers)
            {
                if (id == POwner->id)
...
bool CBattlefield::RemoveEntity(CBaseEntity* PEntity, uint8 leavecode)
{
    ...
    else
    {
        ...
        else if (PEntity->objtype == TYPE_MOB || PEntity->objtype == TYPE_PET)
        {
            ...
            {
                if (dynamic_cast<CPetEntity*>(PEntity)->isAlive() && PEntity->PAI->IsSpawned())
                {
                    dynamic_cast<CPetEntity*>(PEntity)->Die();
                }
