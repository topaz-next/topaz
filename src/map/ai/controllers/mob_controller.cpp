...
void CMobController::TryLink()
{
    ...
    {
        if (PTarget->PPet->objtype == TYPE_PET && (dynamic_cast<CPetEntity*>(PTarget->PPet))->getPetType() == PETTYPE_AVATAR)
        {
    ...
    {
        (dynamic_cast<CMobEntity*>(PMob->PPet))->PEnmityContainer->AddBaseEnmity(PTarget);
    }
    ...
    {
        CMobEntity* PMaster = dynamic_cast<CMobEntity*>(PMob->PMaster);
...
bool CMobController::CanDetectTarget(CBattleEntity* PTarget, bool forceSight)
{
    ...
    if ((detects & DETECT_MAGIC) && PTarget->PAI->IsCurrentState<CMagicState>() &&
        dynamic_cast<CMagicState*>(PTarget->PAI->GetCurrentState())->GetSpell()->hasMPCost())
    {
        return CanSeePoint(PTarget->loc.p);
    }
...
void CMobController::DoCombatTick(time_point tick)
{
    ...
    if (PMob->m_OwnerID.targid != 0 && dynamic_cast<CCharEntity*>(PMob->GetEntity(PMob->m_OwnerID.targid))->PClaimedMob != static_cast<CBattleEntity*>(PMob))
    {
    ...
    HandleEnmity();
    PTarget = dynamic_cast<CBattleEntity*>(PMob->GetEntity(PMob->GetBattleTargetID()));
    ...
    if (IsSpellReady(currentDistance) && TryCastSpell())
    {
        return;
    }
    if (m_Tick >= m_LastMobSkillTime && tpzrand::GetRandomNumber(10000) <= PMob->TPUseChance() && MobSkill())
    {
        return;
    }
...
void CMobController::Move()
{
    ...
    {
        CMobEntity* posShare = dynamic_cast<CMobEntity*>(PMob->GetEntity(PMob->getMobMod(MOBMOD_SHARE_POS) + PMob->targid, TYPE_MOB));
        ...
        if (PMob->speed != 0 && PMob->getMobMod(MOBMOD_NO_MOVE) == 0 && m_Tick >= m_LastSpecialTime)
        {
            ...
            else if (CanMoveForward(currentDistance))
            {
                ...
                if (!PMob->PAI->PathFind->IsFollowingPath())
                {
                    ...
                    {
                        for (auto PSpawnedMob : dynamic_cast<CCharEntity*>(PTarget)->SpawnMOBList)
                        {
...
void CMobController::HandleEnmity()
{
    ...
    {
        ChangeTarget(dynamic_cast<CMobEntity*>(PMob->GetEntity(PMob->getMobMod(MOBMOD_SHARE_TARGET), TYPE_MOB))->GetBattleTargetID());
...
void CMobController::DoRoamTick(time_point tick)
{
    ...
    if (PMob->m_OwnerID.id != 0 && !(PMob->m_roamFlags & ROAMFLAG_IGNORE))
    {
        // i'm claimed by someone and need hate towards this person
        PTarget = dynamic_cast<CBattleEntity*>(PMob->GetEntity(PMob->m_OwnerID.targid, TYPE_PC | TYPE_MOB | TYPE_PET | TYPE_TRUST));
    ...
    //#TODO
    if (PMob->GetDespawnTime() > time_point::min() && PMob->GetDespawnTime() < m_Tick)
    {
        Despawn();
        return;
    }
