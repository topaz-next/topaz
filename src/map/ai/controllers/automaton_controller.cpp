...
void CAutomatonController::DoCombatTick(time_point tick)
{
    ...
    PTarget = dynamic_cast<CBattleEntity*>(PAutomaton->GetEntity(PAutomaton->GetBattleTargetID()));
...
bool CAutomatonController::TrySpellcast(const CurrentManeuvers& maneuvers)
{
    ...
    switch (PAutomaton->getHead())
    {
        ...
        case HEAD_STORMWAKER:
        {
            ...
            if (!lowHP && maneuvers.ice && TryElemental(maneuvers)) // Ice -> Nuke
            {
                m_LastElementalTime = m_Tick;
                return true;
            }
            ...
            if (!lowHP && !maneuvers.ice && TryElemental(maneuvers))
            {
                m_LastElementalTime = m_Tick;
                return true;
            }
        ...
        case HEAD_SPIRITREAVER:
        {
            ...
            if (maneuvers.dark && TryEnhance())
            {
                m_LastEnhanceTime = m_Tick;
                return true;
            }
            if ((maneuvers.dark || PAutomaton->GetHPP() <= 75 || PAutomaton->GetMPP() <= 75) &&
...
bool CAutomatonController::TryHeal(const CurrentManeuvers& maneuvers)
{
    ...
    if (maneuvers.light && !PCastTarget && PAutomaton->getHead() == HEAD_SOULSOOTHER && PAutomaton->PMaster->PParty) // Light + Soulsoother head -> Heal party
    {
        if (PMob)
        {
            ...
            dynamic_cast<CCharEntity*>(PAutomaton->PMaster)->ForPartyWithTrusts([&](CBattleEntity* PMember) {
    ...
    if (PCastTarget)
    {
        ...
        if (missinghp > 600 && Cast(PCastTarget->targid, SpellID::Cure_V))
        {
            return true;
        }
        if (missinghp > 350 && Cast(PCastTarget->targid, SpellID::Cure_IV))
        {
            return true;
        }
        if (missinghp > 190 && Cast(PCastTarget->targid, SpellID::Cure_III))
        {
            return true;
        }
        if (missinghp > 120 && Cast(PCastTarget->targid, SpellID::Cure_II))
        {
            return true;
        }
        if (Cast(PCastTarget->targid, SpellID::Cure))
        {
            return true;
        }
    }
...
{
    const auto& PSpell   = autoSpellList[spell];
