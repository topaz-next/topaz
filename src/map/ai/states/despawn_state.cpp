...
{
    if (_PEntity->status != STATUS_DISAPPEAR && !(dynamic_cast<CMobEntity*>(_PEntity)->m_Behaviour & BEHAVIOUR_NO_DESPAWN))
    {
...
{
    if (tick > GetEntryTime() + 3s && !IsCompleted() && !(dynamic_cast<CMobEntity*>(m_PEntity)->m_Behaviour & BEHAVIOUR_NO_DESPAWN))
    {
        dynamic_cast<CMobEntity*>(m_PEntity)->OnDespawn(*this);
        Complete();
    }
    return IsCompleted();
}
