...
void CTargetFind::findWithinArea(CBattleEntity* PTarget, AOERADIUS radiusType, float radius, uint8 flags)
{
    ...
    else
    {
        ...
        if (m_findFlags & FINDFLAGS_HIT_ALL || (m_findType == FIND_MONSTER_PLAYER && (dynamic_cast<CMobEntity*>(m_PBattleEntity))->CalledForHelp()))
        {
...
void CTargetFind::addAllInParty(CBattleEntity* PTarget, bool withPet)
{
    ...
    {
        dynamic_cast<CCharEntity*>(PTarget)->ForPartyWithTrusts([this, withPet](CBattleEntity* PMember) { addEntity(PMember, withPet); });
    }
    else
    {
...
void CTargetFind::addAllInEnmityList()
{
    ...
    {
        CMobEntity*   mob        = dynamic_cast<CMobEntity*>(m_PBattleEntity);
    ...
    if (allegiance == ALLEGIANCE_PLAYER)
    {
        if (PTarget && PTarget->objtype == TYPE_PC)
        {
            ...
            for (const auto& list : { PChar->SpawnPCList, PChar->SpawnPETList })
            {
                ...
                {
                    CBattleEntity* PBattleEntity = dynamic_cast<CBattleEntity*>(pair.second);
...
{
    CBattleEntity* PTarget = dynamic_cast<CBattleEntity*>(m_PBattleEntity->GetEntity(actionTargetID, TYPE_MOB | TYPE_PC | TYPE_PET | TYPE_TRUST));
